"""Thesis full-dataset worker — runs beam search on all AIME problems.

Called by compare_full.py via subprocess. Do NOT run directly.
Usage: python worker_full_thesis.py <input.json> <output.json>
"""
...
def run(test_cases):
    ...
    for i, prob in enumerate(problems):
        ...
        print(f"  Thesis problem {i+1}/{len(problems)} done ({wall_time:.1f}s)", flush=True)
