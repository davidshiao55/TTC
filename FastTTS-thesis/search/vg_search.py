...
def _vg_search(
    ...
    """VG search implementation with two-stage max_tokens approach."""
    
    # Define two-stage max_tokens values
    # Stage 1: Small max_tokens for initial exploration
    # Stage 2: Large max_tokens for final generation
    stage1_max_tokens = min(64, search_config.max_tokens // 4)  # Small max_tokens
    stage2_max_tokens = search_config.max_tokens  # Original max_tokens
    
    # Pre-compute sampling parameters for both stages
    ...
    for i in tqdm(range(search_config.num_iterations), desc="VG search iterations"):
        ...
        if len(active_beams) != search_config.n:
            ...
            # If prefix-aware scheduling is enabled, place duplicate beams adjacent
            if getattr(generator.config, 'prefix_aware_scheduling', False):
                ...
                # Original logic: place duplicates at the end
        ...
        # Use appropriate sampling parameters based on stage
        if i < 3:
            # Stage 1: Use small max_tokens for initial exploration
            current_sampling_params = stage1_sampling_params
        elif i == search_config.num_iterations - 1:
            # Final iteration: Use large max_tokens for final generation
            current_sampling_params = final_sampling_params
        else:
            # Middle iterations: Use large max_tokens for continuation
...
def vg_search(
    ...
    # NVTX profiling for entire VG search
    nvtx.range_push("Total")
    completed_beams, total_generator_latency_s, total_verifier_latency_s, n_generator_latency_s, n_verifier_latency_s, total_num_tokens, n_completion_tokens, extended_tokens_list = _vg_search(problems, search_config, generator, verifier)
    nvtx.range_pop()
