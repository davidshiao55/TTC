...
def _dynamic_branching_search(
    ...
    """Dynamic branching search implementation with score-based beam duplication."""
    ...
    for i in tqdm(range(search_config.num_iterations), desc="Dynamic branching search iterations"):
        ...
        # Dynamic branching: duplicate beams based on their scores
        if len(active_beams) != search_config.n:
            # Calculate scores for active beams to determine duplication ratios
            if i == 0:
                # For first iteration, use uniform distribution
                beam_scores = np.ones(len(active_beams))
            else:
                # Use aggregated scores from previous iterations
                beam_scores = np.array([
                    aggregate_scores(b.all_scores, search_config.agg_strategy) 
                    for b in active_beams
                ])
                # Ensure all scores are positive for proper distribution
                beam_scores = beam_scores - np.min(beam_scores) + 1e-6
            
            # Calculate duplication ratios based on scores
            total_score = np.sum(beam_scores)
            target_beams = search_config.n
            
            # Calculate how many duplicates each beam should get
            duplication_ratios = beam_scores / total_score * target_beams
            duplication_counts = np.floor(duplication_ratios).astype(int)
            
            # Distribute remaining beams to highest scoring beams
            remaining_beams = target_beams - np.sum(duplication_counts)
            if remaining_beams > 0:
                # Sort by fractional part of duplication ratio (descending)
                fractional_parts = duplication_ratios - duplication_counts
                top_indices = np.argsort(fractional_parts)[::-1][:remaining_beams]
                for idx in top_indices:
                    duplication_counts[idx] += 1
            
            # Create duplicated beams
            final_beams = []
            for beam_idx, beam in enumerate(active_beams):
                num_duplicates = duplication_counts[beam_idx]
                if num_duplicates > 0:
                    # Add the original beam
        ...
        # Prune beams more efficiently - keep top beams based on scores
...
def dynamic_branching_search(
    ...
    # NVTX profiling for entire dynamic branching search
    nvtx.range_push("Total")
    completed_beams, total_generator_latency_s, total_verifier_latency_s, n_generator_latency_s, n_verifier_latency_s, total_num_tokens, n_completion_tokens, extended_tokens_list = _dynamic_branching_search(problems, search_config, generator, verifier)
    nvtx.range_pop()
