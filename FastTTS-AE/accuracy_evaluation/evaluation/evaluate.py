...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False, top_n=1):
    ...
    # Handle new format from my_test.jsonl
    # New format: has 'solutions', 'pred', 'reference_answer'
    for sample in samples:
        sample['gt'] = str(sample['reference_answer'])  
        # sample['pred'] = [extract_answer(sample['solutions']['pred'][0], data_name)]   
    ...
    for sample in samples:
        ...
        pass_at_n = [max(sample['score'])]
        score_mat.append(pass_at_n)
        idx += len(sample['pred'])
...
def parse_args():
    ...
    parser.add_argument("--top_n", type=str, default=None, help="Only evaluate top N candidates")
...
if __name__ == "__main__":
    ...
    evaluate(data_name=args.data_name, prompt_type=args.prompt_type, file_path=args.file_path,
             max_num_samples=args.max_num_samples, execute=args.execute, top_n=int(args.top_n))
